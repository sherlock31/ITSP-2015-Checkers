...
//  
//  Motor YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY
//
//
...
Stepper stepperY(200,9,10,11,12);
void setup() {
  /* initialize serial                                       */
  Serial.begin(115200);
  ...
  while(toOrigin<)
  {
    stepperY.step(-10);
    delay(5);
    toOrigin = analogRead(A4);
    Serial.println(toOrigin);
  }*/
}
  ...
  if (Serial.available() >0) {
    ...
    switch (s) {
      ...
      if (val>47 && val<50) {                /* calculate value        */ 
        if(pin==7)                                                                   ///// pin == 7 means stepper direction
        {
