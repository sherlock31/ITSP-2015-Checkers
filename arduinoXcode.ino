//  This code if modified version of MATLAB Arduino support pde ADIOE*
//  
//  pin 7 is used as direction input of stepper motor and pin 6(PWM) is used for number of steps
//  bool done is used to indicate that stepper instructions are ready 
//  
//  pin 2 is used for servo up / down signal and 3 is for control
//
//  Arduino xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
//  
//A0  Mode Pin 0 for Auto & 1 for Manual
//A1  Stepper Pin 0 = Clockwise & 1 =bAnticlockwise
//A2  Servo Pin 0 = PenUp & 1= penDown
//
...
#include <Stepper.h>
Servo servoPen;
Stepper stepperX(200,9,10,11,12);
void setup() {
  /* initialize serial                                       */
  Serial.begin(115200);
  servoPen.attach(3);
  ...
  servoPen.write(45);
  short int mode = analogRead(A0);
  if(mode>900)                                       /// manual mode
  {
    int x;
    int servo;
    while(true)
  ...
  /*short int toOrigin = 0; 
  pinMode(4, INPUT);
  while(toOrigin < 1)
  {
    stepperX.step(10);
    delay(5);
    toOrigin = digitalRead(4);
  }*/
}
...
void loop() {
  ...
  if (Serial.available() >0) {
    ...
    switch (s) {
      ...
      case 21:
      /* the third received value indicates the value 0 or 1 */
      if (val>47 && val<50) {                /* calculate value        */
        if(pin==2)                                                                   //// if pin == 2 means servo operations///
        {
        ...
        else if(pin==7)                                                                   ///// pin == 7 means stepper direction
        {
	...
	Serial.println(agv);       /* send value via serial  */
      ...
      if(pin==6)                                                        ///// pin == 6 means val* comming is number of steps so instruction is complete
      {
        done = true;
      }
      else
